...
document.addEventListener('DOMContentLoaded', () => {
  ...
  btnget.addEventListener('click', async e => {
    ...
    const countdowninterval = setInterval(() => {                  // tạo interval đếm ngược mỗi giây
      ...
      if (remaining <= 0) {                                        // khi hết thời gian
        ...
        countdown.textContent = 'you can now resend the otp.';     // hiển thị thông báo có thể gửi lại
        // todo: thêm nút gửi lại nếu cần
      }
    }, 1000);

    console.log('otp đã được gửi đến email:', email.value);       // log ra email để test
  });
  ...
  btnverifyotp.addEventListener('click', e => {
    ...
    alert('verify successful!');                                   // xác minh thành công
  ...
  btnreset.addEventListener('click', e => {
    ...
    if (password.value !== confirmpassword.value) {                // kiểm tra khớp mật khẩu
      alert('passwords do not match!');                            // báo lỗi nếu không khớp
    ...
    window.location.href = 'log_in.html';                          // chuyển đến trang đăng nhập
  });
});
